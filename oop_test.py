...
if len(sys.argv) == 2 and sys.argv[1] == "english": #if length of argument is 2, and place 1 is "english", PHRASE_FIRST = True
    PHRASE_FIRST = True
...
def convert(snippet, phrase): #defines method convert with three lists: class_names, other_names, param_names.
    class_names = [w.capitalize() for w in random.sample(WORDS, snippet.count("%%%"))] #takes random sample of WORDS, capitalizes WORDS. counts number of %%% inside that key. class_names will be a random list of words in the size of the count of %%%
